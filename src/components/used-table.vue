<template>
    <div class="used-table">
        <h2 class="h3">Used substances</h2>

        <div class="box used-table__overview">
            <div class="used-table__row">
                <span class="used-table__head">Type</span>
                <span class="used-table__head">Time</span>
                <span class="used-table__head">Amount</span>
            </div>

            <div class="used-table__row" v-for="substance in substances">
                <span class="used-table__type">
                    ...
                    {{ capitilize(substance.type) }}
                </span>

                <span class="used-table__time">{{ substance.time }}</span>

                <span class="used-table__amount">{{ substance.amount / 10 }} cl</span>
            </div>

            <a class="used-table__more" href="#">Show more</a>
        </div>
    </div>
</template>
...
export default {
    ...
    created () {
        ...
        axios.post("http://127.0.0.1:3000/substances/", {
            username: "ruben"
        }).then((res) => this.substances = res.data).catch((error) => {
            console.error(error);
        });
    }
}
